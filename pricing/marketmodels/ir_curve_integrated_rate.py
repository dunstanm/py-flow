...
import math
from pricing.instruments.ir_scheduling import year_fraction, DayCountConvention
from reactive.traced import exp as _texp, log as _tlog
...
class IntegratedShortRateCurve(Storable, CurveBase):
    ...
    def set_rates_numerical(self, rates: np.ndarray):
        ...
        for i, r in enumerate(rates):
            # Bypass Storable.__setattr__ to avoid redundant reaktiv ticks 
            # during numerical solver iterations. The final result will be 
            # pushed via the reaktiv Signal at the end of the solve.
            object.__setattr__(pts[i], 'fitted_rate', float(r))
    ...
    def _compute_coefs(self) -> list[tuple[float, float, float, float]]:
        ...
        # Force plain floats to avoid accidental symbolic tree building during numerical path
        R = [float(p.rate) for p in pts]
        T = [float(p.tenor_years) for p in pts]
        I_vals = [float(R[i] * T[i]) for i in range(n)]
    ...
    def df_array(self, tenors: list) -> list[float]:
        """Batch numerical discount factors — O(N + M) implementation."""
        if not tenors:
            return []
        tenors = [self._t(t) for t in tenors]
    ...
    def fwd_array(self, tenors: list, period: float = 1.0) -> list[float]:
        ...
        R = [float(p.rate) for p in pts]
        T = [float(p.tenor_years) for p in pts]
        I_vals = [float(R[k] * T[k]) for k in range(n)]
    ...
    def fwd_at(self, tenor, period: float = 1.0) -> float:
        """Forward rate: (I(t+p) - I(t)) / tau."""
        import datetime
        from pricing.instruments.ir_scheduling import year_fraction, DayCountConvention
        
        t_start = self._t(tenor)
        if isinstance(tenor, datetime.date) and isinstance(period, datetime.timedelta):
            t_end = self._t(tenor + period)
            dc = self.day_counter if getattr(self, "day_counter", None) else DayCountConvention.Act365Fixed
            tau = year_fraction(tenor, tenor + period, dc)
        else:
            t_end = self._t(tenor) + period
            tau = period

        I_start = self._I_at(t_start)
        I_end = self._I_at(t_end)
        if tau <= 0:
            return 0.0
        return (I_end - I_start) / tau
    ...
    def interp(self, t) -> "Expr":
        """Build an Expr tree mapping curve variables to the Integral evaluation."""
        t = self._t(t)
        cache = getattr(self, '_interp_expr_cache', None)
        if cache is None:
            cache = {}
            object.__setattr__(self, '_interp_expr_cache', cache)
        if t in cache:
            return cache[t]
        ...
        for i in range(n - 1):
            if T[i] <= t <= T[i + 1]:
                tau_expr = Const(t - T[i])
                expr = V_c0[i] + V_c1[i]*tau_expr + V_c2[i]*tau_expr*tau_expr + V_c3[i]*tau_expr*tau_expr*tau_expr
                cache[t] = expr
                return expr
    ...
    def _df_expr(self, t) -> "Expr":
        """Build a cached Expr tree for DF(t) = exp(-I(t))."""
        t = self._t(t)
        cache = getattr(self, '_df_expr_cache', None)
        if cache is None:
            cache = {}
            object.__setattr__(self, '_df_expr_cache', cache)
        if t in cache:
            return cache[t]
        ...
        expr = Exp(-I_expr)
        print(f"  [_DF_EXPR] t={t} expr_vars={expr.variables}")
        cache[t] = expr
        return expr
    ...
    def df(self, t):
        ...
        """
        from reactive.traced import _is_tracing, _is_building, _tracing_depth_val
        if _is_tracing():
    ...
    def fwd(self, start, end) -> "Expr":
        ...
        if isinstance(start, datetime.date) and isinstance(end, datetime.date):
            dc = self.day_counter if getattr(self, "day_counter", None) else DayCountConvention.Act365Fixed
            dt = year_fraction(start, end, dc)
        else:
            dt = float(end) - float(start)

        t_start = self._t(start)
        t_end = self._t(end)
