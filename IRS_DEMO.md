...
This does not yet try to provide accurate pricing, careful scheduling, tidy instrument heirarchies.  The focus is on the calculation engine scaling. 

More detailed documentation can be found in the docs folder:
**👉 Expression Trees [docs/architecture/reactive_expr_dag.md](docs/architecture/reactive_expr_dag.md)**
...
| **Instrument** | `InterestRateSwap` | Reactive valuation via `@computed` & `@traceable` |
...
│   └── traceable.py      # @traceable (Tracing logic for complex DAGs)
├── marketmodel/
...
2. **`@traceable` (Execution Tracing)**: Executes the function top-down at runtime using `Expr` math overloads (intercepting `__add__`, `__mul__`, etc.) to explicitly track and trace the mathematical graph.
...
* **The Tracing Approach (`TorchDynamo`)**: In PyTorch 2.0, they introduced **TorchDynamo**, which uses "JIT Tracing". Instead of reading source code, it hooks into Python's native execution. When you run a function, Torch tracks operations dynamically to build the graph (just like our `@traceable` nodes do). If it hits something it doesn't understand, it does a "graph break" and falls back to standard Python.
...
**The Tracing / Operator Overloading Frameworks (Like our `@traceable`)**
