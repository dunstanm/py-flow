...
def test_quantlib_parity():
    """Verify XCCY OIS Parity against QuantLib."""
    ...
    ts = [1, 2, 3, 5, 10]
    usd_py_r = [DualExpr(0.04) for _ in ts]
    eur_py_r = [DualExpr(0.03) for _ in ts]
    c1 = DualCurve(ts, eur_py_r)
    c2 = DualCurve(ts, usd_py_r)
    
    py_swap = PythonXCCYOIS(tenor_yrs, "EUR", c1, "USD", c2, 0.0020, 1.10, py_val_dt)
    py_total_npv = py_swap.npv().value
    
    assert abs(ql_total_npv - py_total_npv) < 1.0

def test_python_analytic_risk_parity():
    """Verify Python XCCY Risk Consistency (Analytic vs Numerical)."""
    ...
    # Analytic Risk
    eur_py_r_dual = [DualExpr(eur_rate) for _ in ts]
    eur_py_r_dual[3] = DualExpr(eur_rate, der=np.array([1.0]))
    c1_dual = DualCurve(ts, eur_py_r_dual)
    
    anal_risk = PythonXCCYOIS(tenor_yrs, "EUR", c1_dual, "USD", c2, basis, fx, val_dt).npv().der[0]
    
    # Numerical Risk
    bump = 1e-4
    eur_py_r_up = [eur_rate for _ in ts]; eur_py_r_up[3] += bump
    v_up = PythonXCCYOIS(tenor_yrs, "EUR", DualCurve(ts, [DualExpr(r) for r in eur_py_r_up]), "USD", c2, basis, fx, val_dt).npv().value
    eur_py_r_dn = [eur_rate for _ in ts]; eur_py_r_dn[3] -= bump
    v_dn = PythonXCCYOIS(tenor_yrs, "EUR", DualCurve(ts, [DualExpr(r) for r in eur_py_r_dn]), "USD", c2, basis, fx, val_dt).npv().value
    num_risk = (v_up - v_dn) / (2 * bump)
    
    assert abs(anal_risk - num_risk) < 1e-3

if __name__ == "__main__":
    if HAS_QUANTLIB:
        test_quantlib_parity()
    test_python_analytic_risk_parity()
