...
def test_verify_xccy_ois_parity():
    ...
    # Assert parity
    assert abs(ql_total_npv - py_total_npv) < 1.0, f"XCCY OIS Parity failed: {abs(ql_total_npv - py_total_npv)}"
    ...
    assert abs(anal_risk - num_risk) < 1e-2, f"Analytic risk {anal_risk} != numerical {num_risk}"

if __name__ == "__main__":
    if HAS_QUANTLIB:
        test_verify_xccy_ois_parity()
    else:
