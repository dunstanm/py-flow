...
def mock_traceable(fn=None, **kwargs):
    if fn is None:
        return lambda f: f
    return fn

mock_reactive.computed = mock_computed
mock_reactive.traceable = mock_traceable
...
for mod in ["reactive", "reactive.computed", "reactive.traceable", "reactive.expr"]:
...
sys.modules["pricing.instruments.ir_swap_fixed_floatapprox"] = MagicMock()
sys.modules["pricing.instruments.portfolio"] = MagicMock()
