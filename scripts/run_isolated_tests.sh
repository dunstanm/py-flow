...
#   nohup ./scripts/run_isolated_tests.sh > /dev/null 2>&1 &
set -u
...
# 0. Environment Setup
echo "=> [0/4] Preparing python virtual environment with uv..." | tee -a "$LOG_FILE"
if ! command -v uv &> /dev/null; then
    echo "ERROR: 'uv' is not installed. Please install uv and try again." | tee -a "$LOG_FILE"
    exit 1
fi
if [ ! -d ".venv" ]; then
    echo "   Creating fresh .venv..." | tee -a "$LOG_FILE"
    uv venv .venv >> "$LOG_FILE" 2>&1
fi
echo "   Syncing all dependencies from pyproject.toml..." | tee -a "$LOG_FILE"
uv sync --extra dev --extra streaming --extra marketdata --extra timeseries --extra lakehouse --extra media --extra ai --extra agents --extra quantlib >> "$LOG_FILE" 2>&1
...
run_test() {
    ...
    timeout "${TIMEOUT_SECS}s" uv run python3 -m pytest "$target_file" -v $PYTEST_ARGS >> "$LOG_FILE" 2>&1
    local exit_code=$?
